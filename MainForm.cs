...
namespace SoftBill
{
    public partial class MainForm : Form
    {
        ...
        private void InitializeComPorts()
        {
            ...
            if (ports.Length > 0)
                _comPortComboBox.SelectedIndex = 0;
        }
        ...
        {
            if (message.Length < 6) return "Invalid message length";
            ...
            try
            {
                ...
                else if (message.Length >= 11 && message[1] == 0x0B) // Slave message
                {
                    ...
                    // Decode state
                    if ((state & 0x01) != 0) parts.Add("Idling");
                    if ((state & 0x02) != 0) parts.Add("Accepting");
                    if ((state & 0x04) != 0) parts.Add("Escrowed");
                    if ((state & 0x08) != 0) parts.Add("Stacking");
                    if ((state & 0x10) != 0) parts.Add("Stacked");
                    if ((state & 0x20) != 0) parts.Add("Returning");
                    if ((state & 0x40) != 0) parts.Add("Returned");

                    // Decode status/events
                    if ((status & 0x01) != 0) parts.Add("Cheat");
                    if ((status & 0x02) != 0) parts.Add("Reject");
                    if ((status & 0x04) != 0) parts.Add("Jam");
                    if ((status & 0x08) != 0) parts.Add("StackFull");
                    if ((status & 0x10) != 0) parts.Add("Cassette");

                    // Decode power/value
                    if ((powerValue & 0x01) != 0) parts.Add("PowerUp");
                    if ((powerValue & 0x02) != 0) parts.Add("InvalidCmd");
                    if ((powerValue & 0x04) != 0) parts.Add("Failure");

                    int billValue = (powerValue >> 3) & 0x07;
                    if (billValue > 0) parts.Add($"Bill${GetBillValueText(billValue)}");
                }
